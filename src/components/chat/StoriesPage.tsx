...
import {
    ...
    CircularProgress, useTheme, Button, Select, MenuItem, FormControl
} from '@mui/material';
...
import {useSrs} from "../a-srsNotes/SrsContext.tsx"; // Adjust path if needed
...
const WordPopup = ({entry, anchor, onClose, storyId, chunkIndex}: WordPopupProps) => {
    ...
    useEffect(() => {
        const handler = (e: MouseEvent) => {
            ...
            // Ignore interactions with the MUI Select dropdown.
            // MUI renders the dropdown menu in a Portal outside of the popup's DOM tree,
            // so normal `.contains()` checks fail and prematurely close the popup.
    ...
    const handleAddToDeck = async () => {
        ...
        try {
            ...
            // 409 Conflict means the card already exists in this deck — treat as success!
...
interface RenderedChunkProps {
    chunk: StoryChunk;
    chunkIndex: number;
    storyId: string; // NEW: Needed for popup
    onWordClick: (entry: VocabEntry, anchor: { x: number; y: number }, chunkIndex: number) => void; // NEW: Added chunkIndex
}

const RenderedChunk = ({chunk, chunkIndex, storyId, onWordClick}: RenderedChunkProps) => {
    ...
    const handleWordClick = (word: string, e: React.MouseEvent) => {
        ...
        onWordClick(entry, {x: rect.left, y: rect.bottom}, chunkIndex); // Pass chunkIndex up
    };
            ...
            {paragraphs.map((para, pi) => {
                ...
                return (
                    <Typography
                        ...
                        component="p"
                        sx={{
                            mb: 2, lineHeight: 1.95, fontSize: '1.05rem',
                        ...
                        {tokens.map((token, ti) => {
                            ...
                            return (
                                <Box
                                    ...
                                    sx={hasEntry ? {
                                        cursor: 'pointer', borderBottom: '1px dotted',
                                        borderColor: alpha(theme.palette.primary.main, 0.5), borderRadius: '2px',
                                        transition: 'all 0.15s ease',
                                        '&:hover': {
                                            bgcolor: alpha(theme.palette.primary.main, 0.12),
                                            borderColor: 'primary.main', borderBottomStyle: 'solid',
                                        },
                                    } : {}}
                                >
                                    {token}
...
const StoryReader = ({story, onBack, onDeleted}: StoryReaderProps) => {
    ...
    const [explanationPanel, setExplanationPanel] = useState<{
        selectedText: string; content: string; isStreaming: boolean;
    } | null>(null);
    ...
    const handleTextSelection = useCallback(() => {
        ...
        setSelectionButton({text, x: rect.left + rect.width / 2, y: rect.top - 8});
    }, []);
    ...
    const handleExplain = useCallback(async (text: string) => {
        ...
        try {
            ...
            while (true) {
                ...
                setExplanationPanel(prev => prev ? {...prev, content: prev.content + chunk} : prev);
            }
            setExplanationPanel(prev => prev ? {...prev, isStreaming: false} : prev);
        } catch (e) {
    ...
    return (
        <Box sx={{display: 'flex', flexDirection: 'column', height: '100%', position: 'relative'}}>
            ...
            <Box sx={{
                ...
                <Box sx={{flex: 1, minWidth: 0}}>
                    <Typography variant="subtitle1" fontWeight={700} noWrap
                                color="text.primary">{story.title}</Typography>
                ...
                <Tooltip title="Click any underlined word to see its definition">
                    <IconButton size="small" sx={{color: 'text.secondary'}}><InfoOutlinedIcon
            ...
            <Box onScroll={handleScroll} onMouseUp={handleTextSelection} sx={{
                flex: 1, overflowY: 'auto', px: {xs: 2, sm: 4, md: 8}, py: 4,
                ...
                {Array.from({length: story.chunk_count}, (_, i) => i)
                    ...
                    .map(i => {
                        ...
                        return (
                            <Box key={i} ref={el => {
                                if (el) chunkRefs.current.set(i, el); else chunkRefs.current.delete(i);
                            }}>
                                {i > 0 && (
                                ...
                                {chunk ? (
                                    <RenderedChunk
                                        ...
                                        chunkIndex={i}
                                        storyId={story.id}
                                        onWordClick={(entry, anchor, chunkIndex) => setPopup({
                                            entry,
                                            anchor,
                                            chunkIndex
                                        })}
            ...
            {selectionButton && (
                <Box onClick={() => handleExplain(selectionButton.text)} sx={{
                    position: 'fixed', left: selectionButton.x, top: selectionButton.y,
                    transform: 'translate(-50%, -100%)', zIndex: 9000, px: 1.5, py: 0.6, borderRadius: 1.5,
                    bgcolor: '#0f172a', border: '1px solid', borderColor: alpha(theme.palette.primary.main, 0.6),
                    color: 'primary.main', fontSize: '0.75rem', fontWeight: 700, letterSpacing: '0.04em',
                    cursor: 'pointer', userSelect: 'none',
                    boxShadow: `0 4px 20px rgba(0,0,0,0.4), 0 0 0 1px ${alpha(theme.palette.primary.main, 0.2)}`,
                    transition: 'all 0.12s ease', display: 'flex', alignItems: 'center', gap: 0.75,
                    '&:hover': {bgcolor: alpha(theme.palette.primary.main, 0.15), borderColor: 'primary.main'},
                    '&::after': {
                    ...
                    <AutoStoriesIcon sx={{fontSize: 13}}/> Explanation
                </Box>
            )}
            ...
            <Box sx={{
                ...
                height: explanationPanel ? '38%' : 0, minHeight: explanationPanel ? 200 : 0,
                transition: 'height 0.3s cubic-bezier(0.4, 0, 0.2, 1)', overflow: 'hidden', zIndex: 100,
                bgcolor: '#080f1a', borderTop: explanationPanel ? '1px solid' : 'none',
                borderColor: alpha(theme.palette.primary.main, 0.2), display: 'flex', flexDirection: 'column',
            }}>
                {explanationPanel && (
                    <>
                        <Box sx={{
                            px: 2.5, py: 1.25, display: 'flex', alignItems: 'flex-start',
                            justifyContent: 'space-between', gap: 2, borderBottom: '1px solid',
                            borderColor: alpha('#fff', 0.06), flexShrink: 0,
                        }}>
                            <Box sx={{flex: 1, minWidth: 0}}>
                                <Typography variant="caption" sx={{
                                    color: 'primary.main', fontWeight: 700, fontSize: '0.65rem',
                                    letterSpacing: '0.08em', textTransform: 'uppercase', display: 'block', mb: 0.25,
                                ...
                                <Typography variant="caption" sx={{
                                    ...
                                    whiteSpace: 'nowrap',
                                }}>"{explanationPanel.selectedText}"</Typography>
                            </Box>
                            <IconButton size="small" onClick={() => setExplanationPanel(null)}
                        ...
                        <Box sx={{
                            ...
                            '&::-webkit-scrollbar': {width: 4}, '&::-webkit-scrollbar-track': {bgcolor: 'transparent'},
...
const StoryCard = ({story, onOpen}: StoryCardProps) => {
    ...
    return (
        <Paper draggable onDragStart={(e) => {
        ...
        }} onDragEnd={() => setDraggedStory(null)} onClick={() => onOpen(story)} elevation={0} sx={{
            p: 2.5, mb: 1.5, mx: 1, borderRadius: 2.5, border: '1px solid', borderColor: alpha('#fff', 0.06),
            ...
            <Box sx={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1}}>
                <Typography variant="subtitle2" fontWeight={700} color="text.primary"
                            sx={{flex: 1, mr: 1, lineHeight: 1.3}}>{story.title}</Typography>
                <Chip label={story.difficulty_label} size="small" sx={{
            ...
            <Stack direction="row" spacing={2} sx={{mt: 1}}>
                <Typography variant="caption"
                            color="text.secondary">{story.word_count?.toLocaleString() ?? '—'} words</Typography>
                <Typography variant="caption"
                            color="text.secondary">{story.unique_word_count?.toLocaleString() ?? '—'} unique</Typography>
                <Typography variant="caption" color="text.secondary">{story.chunk_count} sections</Typography>
            </Stack>
            {story.tags?.length > 0 && (
                <Stack direction="row" spacing={0.5} sx={{mt: 1.5, flexWrap: 'wrap', gap: 0.5}}>
                    {story.tags.map(tag => (<Chip key={tag} label={tag} size="small" sx={{
                        height: 16,
                        fontSize: '0.6rem',
                        bgcolor: alpha('#fff', 0.05),
                        color: 'text.secondary'
...
const UploadPanel = ({onUploaded}: UploadPanelProps) => {
    ...
    const validateFile = (f: File): string | null => {
        if (!ACCEPTED_MIME.includes(f.type) && !ACCEPTED_TYPES.some(ext => f.name.endsWith(ext))) return 'Unsupported file type. Please upload a .txt, .epub, or .pdf file.';
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            if (file) formData.append('file', file); else formData.append('content', content.trim());
    ...
    return (
        <Box sx={{p: 2}}>
            ...
            <TextField fullWidth size="small" placeholder="Title" value={title} onChange={e => setTitle(e.target.value)}
                       sx={{mb: 1.5}} slotProps={{
                input: {
                    sx: {
                        borderRadius: 2,
            ...
            <Box onDragOver={(e) => {
                ...
                setIsDragOver(true);
                 ...
                 onClick={() => !file && fileInputRef.current?.click()} sx={{
                mb: 1.5,
                p: 2,
                borderRadius: 2,
                border: '1px dashed',
                    ...
                    <>
                        ...
                        <Box sx={{flex: 1, minWidth: 0}}>
                            <Typography variant="caption" fontWeight={600} color="success.main" noWrap
                                        sx={{display: 'block'}}>{file.name}</Typography>
                            <Typography variant="caption" color="text.disabled"
                                        sx={{fontSize: '0.65rem'}}>{(file.size / 1024).toFixed(0)} KB
                                · {file.name.split('.').pop()?.toUpperCase()}</Typography>
                        </Box>
                        <IconButton size="small" onClick={(e) => {
                            e.stopPropagation();
                            setFile(null);
                        }} sx={{color: 'text.disabled', flexShrink: 0}}><CloseIcon sx={{fontSize: 14}}/></IconButton>
                    </>
                ) : (
                    <>
                        <UploadFileIcon
                            sx={{fontSize: 20, color: isDragOver ? 'primary.main' : 'text.disabled', flexShrink: 0}}/>
                        <Box>
                            <Typography variant="caption" color={isDragOver ? 'primary.main' : 'text.secondary'}
                                        fontWeight={600}
                                        sx={{display: 'block'}}>{isDragOver ? 'Drop to upload' : 'Drop a file or click to browse'}</Typography>
                            <Typography variant="caption" color="text.disabled" sx={{fontSize: '0.65rem'}}>.txt · .epub
                                · .pdf — max 5MB</Typography>
                        </Box>
                    </>
                )}
            </Box>
            <input ref={fileInputRef} type="file" accept=".txt,.epub,.pdf" style={{display: 'none'}} onChange={(e) => {
            ...
            {!file && (
                <>
                    <Box sx={{display: 'flex', alignItems: 'center', gap: 1, mb: 1.5}}>
                        <Divider sx={{flex: 1, borderColor: alpha('#fff', 0.06)}}/>
                        <Typography variant="caption" color="text.disabled" sx={{fontSize: '0.65rem'}}>or paste
                            text</Typography>
                        <Divider sx={{flex: 1, borderColor: alpha('#fff', 0.06)}}/>
                    </Box>
                    <TextField fullWidth multiline rows={5} size="small" placeholder="Paste German text here…"
                               value={content} onChange={e => setContent(e.target.value)} sx={{mb: 1.5}} slotProps={{
                        input: {
                            sx: {
                                borderRadius: 2,
                                bgcolor: alpha('#fff', 0.04),
            ...
            <Button fullWidth variant="contained" size="small" onClick={handleUpload}
        ...
        <Box sx={{
            ...
            <Box sx={{p: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between'}}>
                ...
                <Tooltip title="Upload a story">
                    <IconButton onClick={() => setShowUpload(!showUpload)} size="small" sx={{
                        ...
                        transition: 'all 0.2s',
                    }}><UploadFileIcon fontSize="small"/></IconButton>
                </Tooltip>
            </Box>
            ...
            <Box sx={{px: 2, pb: 1.5, maxWidth: 300}}>
                <TextField fullWidth size="small" placeholder="Search stories" value={search}
                           onChange={e => setSearch(e.target.value)} slotProps={{
                    input: {
            ...
            <Box sx={{px: 2, pb: 1.5, display: 'flex', gap: 0.75, flexWrap: 'wrap'}}>
                {Object.entries(DIFFICULTY_COLORS).map(([level, color]) => (
                    <Box key={level} sx={{display: 'flex', alignItems: 'center', gap: 0.4}}>
                        ...
                        <Typography variant="caption"
                                    sx={{color, fontSize: '0.65rem', fontWeight: 600}}>{level}</Typography>
                    </Box>
                ))}
            </Box>
            ...
            <Box sx={{flex: 1, overflowY: 'auto'}}>
                {loading ? (
                    Array.from({length: 4}).map((_, i) => (
                        <Box key={i} sx={{mx: 1, mb: 1.5}}><Skeleton variant="rounded" height={80}
                                                                     sx={{borderRadius: 2.5}}/></Box>))
                ) : error ? (
                    ...
                    <Box sx={{textAlign: 'center', py: 6, px: 2}}>
                        ...
                        <Typography variant="body2"
                                    color="text.secondary">{search ? 'No stories match your search' : 'No stories yet — upload one above'}</Typography>
                    </Box>
                ) : (
